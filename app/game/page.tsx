...
export default function GamePage() {
  ...
  const initializeGame = () => {
    // Initialize the sorting algorithm and get all pairs
    ...
    if (allPairs.length > 0) {
      ...
      // Set the next 4 pairs as upcoming
      const upcoming = allPairs.slice(1, 5)
      setUpcomingPairs(upcoming)
    }
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="mb-4 mt-4">
          ...
          <div className="text-right text-sm mt-1 text-gray-300">
            {completedComparisons.current} of {totalComparisons.current} comparisons
          </div>
        </div>
        ...
        <div className="relative h-[200px] w-full flex justify-center">
          {/* Render the stack of upcoming cards */}
          {upcomingPairs.map((pair, index) => {
            const offsetY = 10 + index * 15 // Increasing offset for each card
            const scale = 1 - index * 0.05 // Decreasing scale for each card
            const opacity = 1 - index * 0.2 // Decreasing opacity for each card

            return (
              <div
                key={`stack-${pair[0]}-${pair[1]}`}
                className="absolute w-[80%] h-[120px] bg-gray-800/50 border border-gray-700 rounded-xl shadow-md flex justify-center items-center"
                style={{
              ...
              >
                <div className="flex justify-center items-center gap-4">
                  <div className="w-16 h-16 bg-gray-700/70 rounded-lg flex items-center justify-center overflow-hidden">
                    <Image
                      src={countries[pair[0]].flag || "/placeholder.svg?height=64&width=64"}
                      alt={`Flag preview`}
                      width={64}
                      height={64}
                  ...
                  <div className="text-2xl font-bold text-white opacity-50">VS</div>
                  <div className="w-16 h-16 bg-gray-700/70 rounded-lg flex items-center justify-center overflow-hidden">
                    <Image
                      src={countries[pair[1]].flag || "/placeholder.svg?height=64&width=64"}
                      alt={`Flag preview`}
                      width={64}
                      height={64}
                      className="object-cover opacity-70"
